...
*Living status document. Last updated 2026-08-04 — SESSION CLOSE: `origin/main` was **`f977596`** at the end of the W29 wave work (later doc commits move it — check `git log -1 origin/main`), local in sync, tree clean, build clean, **50/50 floor**, 12/12 screenshots; the backlog's "Ready to build" section is EMPTY (remaining: item 0 Redo — a maintainer decision — plus the B8 smalls and optional B6 library growth). ⚠ W28 (LCD count-in face) was committed with the subject `[W27 f/u]`. (latest: **W29 — `--selftest-popout` hardening** — NOT a confirmed fix, and the docs say so: the intermittent failure W25 saw was **never reproduced** (65 dedicated runs — 40 isolated + 25 in the floor's real tray→popout ordering — all PASS), and the original report had been overwritten by the next gate before the failing leg could be read. Two defensible changes shipped anyway: the deferred phase's fixed `startTimer(300)` became a **condition wait** (poll until both popout resets have actually run — a directly checkable state — with a ~2 s ceiling that verifies anyway on timeout), because waiting a fixed duration for an observable event is a race regardless of this bug; and a failure now **names its failing legs in the PERSISTENT log**, which survives the rest of a floor run (verified by inducing a failure). Measured datum: the resets complete in ~25–50 ms, so the old 300 ms already had ~6× margin — making message-loop starvation a less likely cause than it appeared. Build clean, 50/50 floor, 12/12 screenshots. devlog `devlog/wave-29-popout-hardening.md`. Prior: **W27 — capture count-in (B4)** — the last "ready to build" backlog item, and one carrying a WRONG inherited rationale. W22 deferred it believing "the only audible-count-in path puts the transport into record-mode for the whole Capture session"; reading `TransportControl::record()` (`tracktion_TransportControl.cpp:1483-1489`) shows the count-in is only a **position offset** (`startTime - countInBeats`) plus a forced click range — both reproducible on a plain `play()`. Reachable only THROUGH `record()` is not the same as a property OF record mode (now a CLAUDE.md gotcha). Shipped: `forge::capture::planCountIn` (new header-only `src/engine/CaptureCountIn.h` — pure, engine-free, exhaustively gate-able) + a `ProjectSession` `countingIn` state MUTUALLY EXCLUSIVE with `capturing` (the sampler tick does not run during the pre-roll, so no span opens before the downbeat): rolls the transport from the pre-roll beat with the click forced on, arms on the downbeat, restores the user's click exactly. `isPerformanceCaptureArmed()` true throughout (the Capture toggle stays lit); cancelling mid-count-in disarms, restores the click and stops the transport Forge started. Skip paths keep prior behaviour: already-rolling arms at once; NO count-in configured is byte-identical to pre-B4 (arm now, never touch the transport — W17's passive-observer contract, pinned by its own gate leg). No new UI — it reads `Edit::getNumCountInBeats()`, so the existing transport-bar selector serves record and capture alike. Gate **`--selftest-countin`** (floor **49 -> 50**, 19 legs) with TWO independent negative controls. Build clean, **50/50 floor**, 12/12 screenshots. Its one documented limit was CLOSED in a same-day follow-up: the LCD count-in face was record-only, and now lights for capture too — `LcdInput.captureCountIn` + an OR in the active test, SHARING the click-grid digit derivation verbatim (the arm beat stands in for the punch, which does not exist when nothing is recording); `LcdDisplay::queryCaptureCountIn` is wired by the shell with the same rising-edge latch so `getNumCountInBeats()` stays off the 25 Hz poll. No new gate (floor stays 50): 10 pure-model legs on `--selftest-lcd` + 3 WIRING legs on `--selftest-countin` driving the real `pollNow()` — a negative control showed unwiring the seam leaves `--selftest-lcd` fully green while the wiring legs fail. devlog `devlog/wave-27-capture-countin.md`. Prior: **W26 — instrument assignment (B6)** — the backlog's last open feature item, opened by a **dead-code report**: W24's four self-rendered CC0 melodic voices (PluckBass/Pad/Bell/Clav) had every reference in `src/engine/` and **none** in `src/ui/` or `main.cpp` — in the binary, unreachable. `PluginHost::applyInstrumentPreset` already did the engine work, so W26 built only the missing path: **one seam** (`ProjectSession::setTrackInstrument`), **one catalogue** (`PluginHost::getInstrumentChoices`/`getInstrumentPresetName`, now also the Sampler sound name so a picker label and the loaded sound cannot drift — two inline copies of those strings deleted), and **three surfaces** rendering from it: an `Instrument ▸` submenu appended to the Arrange lane-header menu, a NEW right-click menu on the Session track header (`TrackColumnComponent` gained its first header mouse handler + a static `isInHeaderBand` hit test), and an **INSTRUMENTS** list above the Browser file tree. The Browser list is track-agnostic; the shell picks the target (`instrumentTargetTrack()` — focused Session column, else last-selected Arrange lane, clamped) and the status strip names both voice and track. **No per-slot assignment by design** (the engine is track-level — the W21 first-instrument-wins gotcha / W22 move-to-own-track fix). Gate **`--selftest-instrument`** (floor **48 → 49**) drives BOTH surfaces through their real public entry points, the Browser one WITHOUT stubbing the shell callback (proving browser→shell→seam end to end; a negative control confirmed unwiring the binding flips exactly that leg), and its phase 2 **closes the B3 leftover** — all four melodic voices render non-silent (`weakestPeak ≈ 0.31`, a genuine PASS). Build clean, **49/49 floor**, 12/12 screenshots. devlog `devlog/wave-26-instrument-assignment.md`. Prior: **W25 — multi-stem audio import** — a maintainer brief, filed as backlog **B9** and burned down the same day: dropping N files onto an Arrange lane now lands **one clip per file on N consecutive tracks, named after the files** (it previously imported the first file and discarded the rest). New `ProjectSession::importFilesMultiTrack` + the audio-only `importAudioFilesMultiTrack` convenience, built on W24's `importMidiFileMultiTrack` template (validate before mutating · `getOrInsertAudioTrackAt` grows lanes on demand · a per-file failure does NOT advance the destination index, so no gap lanes · one `markAsChanged` · `onTracksChanged` only if the list grew), plus three things the MIDI seam did not need: a **deterministic filename sort** (the OS drop order is arbitrary — without it no gate could assert lane assignment), **destination-track naming** after the file (guarded: never renames a user-named lane, nor one already holding ARRANGE **or** Session-slot clips), and an explicit **mixed audio+MIDI walk** (each file takes the next index; a `.mid` consumes as many lanes as it lands clips). `TrackLaneComponent::filesDropped` now carries every accepted file instead of returning on the first; the shell does **ONE** save per drop (six would be six undo steps and six trips through the FourOsc redo-wipe defect). New engine gotcha: `AudioTrack::getName()` SYNTHESISES "Track N" when unnamed and `sanityCheckName()` resets a literal "Track N" to empty — the only honest "did the user name this?" read is `state[te::IDs::name]`. Gate `--selftest-stems` (floor **48/48**, 47 → 48; 24 legs; both load-bearing rules negative-controlled — deleting the sort flips 3 legs, advancing on failure isolates the gap-lane leg), 12/12 screenshots unchanged (no new UI state). Also corrected the stale gate/screenshot counts in `README.md` and `docs/ARCHITECTURE.md` (both claimed "sixteen gates / 9 screenshots"; ARCHITECTURE's ProjectSession seam inventory now names all three import layers and defers the gate roster to `tests/SELFTEST.md` rather than mirroring it). ✅ **PUSHED to `origin/main`, tip `062c248`** (verified by `git fetch` after the push; local `main` == `origin/main`, nothing unpushed). ⚠ **`--selftest-popout` failed ONCE** during this wave's first full-floor run and passed 6× since (incl. the pre-change baseline) with no WARN/ERROR — an intermittent window-timing flake, unrelated to this diff, logged as a B8 item rather than fixed. devlog `devlog/wave-25-multistem-import.md`. Prior: **W24 — backlog burn-down** — the first wave driven straight off **[BACKLOG.md](BACKLOG.md)**: five "Ready to build" items in one pass (3 parallel file-disjoint agents on B1/B5/B7 + the orchestrator owning `main.cpp` for B2/B3 and all wiring), plus two standalone same-day landings (B6 groundwork `13a1f0f` — the PluckBass/Pad/Bell/Clav self-rendered CC0 melodic voices + `InstrumentPreset` plumbing, API-only; B8 curated launch-quantise submenu `14c2c61`). Shipped: **(B1)** multi-track MIDI import (`ProjectSession::importMidiFileMultiTrack` over the engine's own `te::readFileToMidiList` parse — one born-audible clip per non-empty source track/channel, CC-only parts skipped; browser filter + double-click + arrange drop now serve `.mid;.midi`; `--selftest-midifile` +6 legs); **(B2)** `--selftest-reload`, the FIRST gate to re-read state from DISK (seed → saveAs → mutate-away in memory → reopen via the real `swapProject` → assert — the mutate-away step defeats the false-pass where a silent reload failure leaves the old in-memory edit); **(B3)** render-audibility legs on `--selftest-demo`/`-drumkit` (deferred phase pumps the Sampler's async ingestion then renders a stem and asserts non-silence — verified genuine PASS, peaks ≈0.55/0.65; closes the W09 ingestion follow-up + W22 drum-render deferral); **(B5)** both W23 trim limits closed (speed-correct audio trim `Δ = Ts/speed − offset`, source-verified against `clipTimeToSourceFileTime`, auto-tempo now a logged decline — a latent mis-trim hole; CC-only MIDI trim); **(B7)** Modulate polish (Edit ▸ Modulate… Ctrl+M + a modulated-parameter accent dot on mixer/tray/Session strips riding existing polls, edge-compared, no new timers; `--selftest-menu` Edit 3→4 + Ctrl+M pin; `--selftest-sessionmixer` +3 indicator legs; the screenshot demo now lights the dot). Build clean, **47/47 floor** (46 → 47: `+reload`), 12/12 screenshots. **Committed locally on `main`, push HELD for maintainer OK**; devlog `devlog/wave-24-backlog-burndown.md`. Remaining on the backlog: item 0 Redo (maintainer A/B/C call), B4 capture count-in, B6's browser→slot interaction, B8 leftovers. Prior: **W23 — hands-on follow-ups** — four maintainer requests from driving the built app: (1) the drawer **piano roll** gained its own zoomable beat→pixel time axis (decoupled from the shared arrange `TimelineView`) + a moving playhead + a clearer bar/beat/sub-beat grid + a global-undo key fallback (`PianoRollView::onUnhandledKey`, so Ctrl+Z fires from inside the roll regardless of focus) — `--selftest-pianoroll` + a `session_pianoroll` screenshot; (2) **"Trim silent start"** on an arrange MIDI clip (`forge::midiedit::trimLeadingSilence` over `Clip::setStart(preserveSync)` — crops the lead-in to the first note, notes keep absolute position, nothing deleted, undoable) — `--selftest-trim`; (3) **tap tempo** source-verified as ALREADY a rolling 4-tap average (no functional change; added a `windowDropsStale` gate leg); (4) a **time-signature editor** (click the LCD "· 4/4" zone → a new `TimeSigPopup` over a new `EngineHelpers::setTimeSigAt` seam; ruler/snap/piano-roll bars follow live) — `--selftest-timesig`. Built via a 3-agent research fan-out → 2-agent file-disjoint implementation → orchestrator consolidation; build clean, **46/46 floor** (43 → 46), 12/12 screenshots. A **third round then knocked out every documented follow-up** (3 more parallel file-disjoint agents; NO new gates — the floor stays 46): the arrange **ruler** right-click inserts a time-sig change at the clicked bar (`ArrangeView::insertTimeSigAtBar`); **audio-clip trim** (`forge::audioedit::trimLeadingSilence`, a −60 dBFS `searchForLevel` scan) so the trim menu serves MIDI and wave clips; a **Trim button** in the piano-roll nav strip; **tap tempo now requires ≥3 taps** (which broke — and the gate caught — the 2-tap `clampHigh` leg); and **both proof gaps closed** (a public `PianoRollView::handleWheel` seam + `LcdDisplay` zone-bounds accessors, so wheel routing and the sig-zone click target are now gated). Engine correction: a clip's `offset` is in EDIT-seconds while a silence scan yields a SOURCE-second, so audio trim **declines on a non-unity speed ratio** rather than mis-trim a stretched clip. **Committed locally on `main`, push HELD for maintainer OK**; devlog `devlog/wave-23-handson-followups.md`. Prior: **W22 — buildable-backlog follow-ups** — piano-roll keyboard nudge (`--selftest-nudge`), MIDI retrospective capture (`ProjectSession::commitRetrospectiveToSlot`, `--selftest-retrocapture`; a QC-caught use-after-free fixed), the "Modulate" LFO UI (Ctrl+M param picker → `addLFO`+`assign`, making the W21 seam usable), LFO config-coverage gate legs, and the per-clip "Move to its own track" fix (`moveSlotClipToOwnTrack` + `--selftest-movetotrack` — the maintainer-chosen resolution for the mixed-clip "first-instrument-wins" silence, the engine being track-level only); build clean, **43/43 floor** (40 → 43), 11/11 screenshots; **PUSHED to origin/main**; capture count-in + a drum-render gate leg DEFERRED (rationale in the devlog); devlog `devlog/wave-22-followups.md`. Prior: **W21 — frontier Wave 9 (LFO modifiers) + a self-rendered drum sampler for Step Clips** — a two-track wave that **completes the 10-wave frontier build program** (Wave 9 was the last one outstanding): a header-only `src/engine/ModifierHelpers.h` LFO seam (`--selftest-modifier`), and Step Clips now born with a `te::SamplerPlugin` of 8 self-rendered CC0 drum one-shots (`--selftest-drumkit`); build clean, **40/40 floor** (38 → 40), 11/11 screenshots, a 2-dimension adversarial QC (both SHIP, 4 minor/doc fixes); **PUSHED to origin/main (W21 tip `87225da`)**; devlog `devlog/wave-21-lfo-drumkit.md`. ⚠ This top-block history was stale at W16 — W17–W20 (frontier Waves 7/8/10 incl. the Step Clips CAPSTONE) and the MIDI-file-import feature also shipped in between; **[HANDOFF.md](HANDOFF.md) is the current cross-wave source**. Prior: **W16 — frontier program Wave 6: W05 QC debt discharge (undo + popout + lifetime hardening)** (PUSHED to origin/main, tip `96b1037`; floor stays **32** — all 6 dimensions extend 4 existing gates, zero new gate names; **all THIRTY-TWO selftests PASS**; building it surfaced a confirmed, unfixed engine defect — `FourOscPlugin`'s mod-matrix flush wipes the redo stack on every save, see the CLAUDE.md gotcha — and QC caught + fixed a real regression in the wave's own piano-roll fix (selection/scroll wiped on unrelated undo/redo); devlog `devlog/wave-16-w05-qc-debt.md`); prior: **W15 — frontier program Wave 5: Session scene lifecycle (rename → delete → reorder)** (PUSHED to origin/main, tip `9cc7f04`; floor 29 → 32 via `--selftest-scenerename` / `-scenedelete` / `-scenereorder`; a 5-dim adversarial QC fixed a MAJOR reorder desync + 2 MINORs; devlog `devlog/wave-15-scene-lifecycle.md`); prior: **W14 — frontier program Wave 4: MIDI quantise** (pushed to origin/main); prior: W13 grid clip primitives, W12 per-clip launch quantise, **W11 — frontier program Wave 1: launcher
