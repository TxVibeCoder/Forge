/*
    ...
    Phase-2 placeholder Label. Lets the user navigate the filesystem and pick audio or MIDI files
    to import: a small "Browser" header sits above a juce::FileTreeComponent, which is backed by a
    DirectoryContentsList scanned on a background TimeSliceThread and filtered to common audio
    extensions (*.wav;*.aif;*.aiff;*.flac;*.ogg;*.mp3) plus MIDI files (*.mid;*.midi).

    Double-clicking an existing file fires onImportFile(file); the shell imports it via
    ProjectSession (the shell's dispatch branches audio-vs-MIDI on the file extension, the same
    branch the Arrange file-drop uses). A dumb view: it owns no project logic, only forwards
    intent via the callback.
