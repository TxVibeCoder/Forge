...
- **Selftest floor** (must pass after any change — **FIFTY** gates. The count has been 50 since **W27**
  (`--selftest-countin`); **W28** (LCD count-in face) and **W29** (popout hardening) both extended existing
  gates IN PLACE and added none. Derive it, don't trust this line:
  `grep -oE '"--selftest[a-z-]*"' src/main.cpp | sort -u | wc -l`.
  ...
  source — see the gotcha below). **W28** (committed with the subject `[W27 f/u]` — the docs use W28
  throughout) extended `--selftest-lcd` (+10 pure-model legs) and
